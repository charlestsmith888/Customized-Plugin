...
if (!function_exists('woo_simple_auction_required')) {
	function woo_simple_auction_required() {
		...
		wp_die('<p>The <strong>WooCommerce Simple Auctions</strong> plugin requires '.$flag.'  version '.$version.' or greater. If you need secure hosting with all requirements for this plugin contact us at <a href="mailto:info@wpgenie.org">info@wpgenie.org</a></p>','Plugin Activation Error',  array( 'response'=>200, 'back_link'=>TRUE ) );
	}
}
	...
	if (!class_exists('WooCommerce_simple_auction')) {
		class WooCommerce_simple_auction {
			...
			public function product_write_panel() {
				...
				echo '	
				<p class="form-field auction_dates_fields">
						...
						<p class="form-field auction_dates_fields">
							<label for="_auction_dates_from"></label>
							<a href="javascript:;" id="endthiscomp" class="button button-primary button-large">Stop Auction</a>
						</p>
...
jQuery('#endthiscomp').click(function() {
	jQuery('input[name="_auction_dates_to"]').val(get_datetime()).attr('readonly', '');;
});
...
Use upper-case characters for mandatory periods, or the corresponding lower-case characters for optional periods, i.e. only display if non-zero. Once one optional period is shown, all the ones after that are also shown.", 'wc_simple_auctions'),
			...
			function woocommerce_simple_auctions_meta_callback() {
				...
				if ($product_data->get_auction_closed() == '3') {?>
						...
						<?php } elseif ($product_data->get_auction_current_bider()) {
					?>
						...
						<?php } elseif ($product_data->get_order_id()) {
							$order = new WC_Order( $product_data->get_order_id() );	
							$order_status = isset($order->status) ? $order->status : __('unknown', 'wc_simple_auctions');?>
					...
					?>
							...
							<p class="reminder-status"><?php _e('Reminder status', 'wc_simple_auctions')?>: <?php if ($product_data->get_stop_mails()) {?><span class="error"><?php _e('Stopped', 'wc_simple_auctions')?></span><?php } else {?><span class="ok"><?php _e('Running', 'wc_simple_auctions')?></span><?php }?> </p>
						<?php }?>
			...
			function woocommerce_simple_auctions_automatic_relist_callback() {
				...
				woocommerce_wp_text_input(array('id' => '_auction_relist_fail_time', 'class' => 'wc_input_price short', 'label' => __('Relist if fail after n hours', 'wc_simple_auctions'), 'type' => 'number', 'custom_attributes' => array(
					'step' => 'any',
					'min' => '0',
				)));
				woocommerce_wp_text_input(array('id' => '_auction_relist_not_paid_time', 'class' => 'wc_input_price short', 'label' => __('Relist if not paid after n hours', 'wc_simple_auctions'), 'type' => 'number', 'custom_attributes' => array(
					'step' => 'any',
					'min' => '0',
				)));
				woocommerce_wp_text_input(array('id' => '_auction_relist_duration', 'class' => 'wc_input_price short', 'label' => __('Relist auction duration in h', 'wc_simple_auctions'), 'type' => 'number', 'custom_attributes' => array(
					'step' => 'any',
					'min' => '0',
				)));
			...
			function auction_payed($order_id, $old_status, $new_status) {
				...
				} elseif($new_status == 'refunded' or $new_status == 'cancelled'){
			...
			*/
			...
			function add_auction_to_user_metafield($data){
