...
<table id="auction-history-table-<?php echo $product->get_id(); ?>" class="auction-history-table">
    <?php 
        ...
        <thead>
            <tr>
                ...
                <th><?php _e('Status', 'wc_simple_auctions') ?></th>
        ...
        <?php if ($product->is_sealed()){
            ...
            foreach ($auction_history as $history_value) {
                ...
                echo "<td class='status'>".strtoupper($history_value->status)."</td>";
	...
	<tr class="start">
        ...
        if ($product->is_started() === TRUE ){
            echo '<td class="date">'.mysql2date($datetimeformat ,$product->get_auction_start_time()).'</td>'; 
