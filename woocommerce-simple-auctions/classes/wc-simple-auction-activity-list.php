...
class wc_simple_auctions_List_Table extends WP_List_Table {    
    ...
    function column_default($item, $column_name){
        ...
        switch($column_name){
            ...
            case 'Change':
                return '<a href="?'.$_SERVER['QUERY_STRING'].'&changeStatus=ture&bid_id='.$item['id'].'">Change</a>';
            case 'date':
            ...
            default:
                return  apply_filters('woocommerce_simple_auctions_activity_column_default',$item[$column_name],$item,$column_name);
        }
    }
    ...
    function prepare_items() {
        ...
        if (isset($_GET['changeStatus'])) {
            $id = $_GET['bid_id'];
            $dateonce = $wpdb->get_row( "SELECT * FROM ".$wpdb->prefix."simple_auction_log WHERE id = ".$id." " );
            $status = '';
            if ($dateonce->status == 'approved') {
                $status = 'unapproved';
            }else{
                $status = 'approved';
                $highestbid = $wpdb->get_row( "SELECT MAX(`bid`) as max FROM `shearree_simple_auction_log` WHERE `status` = 'approved'" );
                if ($dateonce->bid > $highestbid->max   AND !empty($highestbid->max)) {
                    update_post_meta($dateonce->auction_id, '_auction_current_bid', $dateonce->bid);
                }elseif(empty($highestbid->max)){
                    update_post_meta($dateonce->auction_id, '_auction_current_bid', $dateonce->bid);
                }else{
                    echo "1";
                }
            }
            $wpdb->query("UPDATE ".$wpdb->prefix."simple_auction_log SET status='".$status."' WHERE id = '".$id."'  ");
            wp_redirect( $_SERVER['HTTP_REFERER'].'&msg=1' );
        }
        if (!empty($_GET['msg']) and @$_GET['msg'] == '1') {
            echo "<div class='notice notice-success is-dismissible'><p>Bid status has been changed...!</p></div>";
        }
    }
}