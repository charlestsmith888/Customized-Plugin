...
class WC_Bid {
	...
	public function placebid($product_id, $bid) {
		...
		if (!apply_filters( 'woocommerce_simple_auctions_before_place_bid_filter',$product_data))
			return false;
			...
			if ( $product_data->bid_value() <= ($this->bid )) {
				...
				if ($product_data->get_auction_proxy()) {
					...
					if ($this->bid > (float)$product_data->get_auction_max_bid()) {
						...
						wc_add_notice(sprintf(__('Someone placed a higher bid. Try again!', 'wc_simple_auctions'), $product_data -> get_title()),'error');
			...
			if ($product_data->bid_value()  >= $bid) {
				...
				if ($product_data->get_auction_proxy() ) {
					...
					if (  $this->bid < (float)$product_data->get_auction_max_bid() OR  !$product_data->get_auction_max_bid()) {
						...
						wc_add_notice(sprintf(__('Someone placed higher bid. Try again!', 'wc_simple_auctions'), $product_data -> get_title()),'error');
