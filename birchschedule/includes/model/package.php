...
birch_ns( 'birchschedule.model', function( $ns ) {
		...
		$ns->get_appointment_fields = function() {
			$meta_keys = array(
				...
				'_birs_appointment_capacity', '_birs_appointment_apt_time'
			);
			return $meta_keys;
		};
		...
		$ns->get_appointment1on1_custom_fields = function() {
			return array( '_birs_appointment_notes', '_birs_appointment_apt_time' );
		};
		...
		$ns->get_client_fields = function() {
			$client_meta_keys = array(
				...
				'_birs_client_zip', '_birs_client_property_city', '_birs_client_property_street',
				'_birs_client_zip_code', '_birs_client_zip_code', '_birs_client_size_sf', '_birs_client_year_built', '_birs_client_services'
			);
		...
		$ns->get_staff_daysoff = function( $staff_id ) {
			...
			// yahn kam krna he fucking shit


			$fruitList = [];

			foreach (get_post_meta($staff_id , 'disabledate' ) as $fruit)
			{
				$timestamp = strtotime($fruit);
				$dateaaa = date('m/d/Y', $timestamp);

				$fruitList[] .= $dateaaa;
			}

			$daysoff = json_encode($fruitList);

			// $daysoff = '["09\/03\/2017","09\/04\/2017","08\/30\/2017","08\/29\/2017","08\/28\/2017"]';
