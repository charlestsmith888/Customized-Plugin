...
if ( !class_exists( 'YITH_YWRAQ_Frontend' ) ) {
    ...
    class YITH_YWRAQ_Frontend {
        ...
        public function __construct() {
            ...
            add_action( 'woocommerce_after_add_to_cart_button', array( $this, 'add_button_single_page' ), 35 );


            //custom styles and javascripts
            add_action( 'wp_enqueue_scripts', array( $this, 'enqueue_styles_scripts' ) );

            add_action( 'woocommerce_after_add_to_cart_button', array( $this, 'hide_add_to_cart_single' ), 10 );
        ...
        public function hide_add_to_cart_single() {
	        ...
	        if ( get_option( 'ywraq_hide_add_to_cart' ) == 'yes' ) {
		        if ( isset( $product ) && $product && $product->is_type('variable') ) {
			        ...
			        $priority = has_action( 'woocommerce_after_add_to_cart_button', 'woocommerce_template_single_add_to_cart' );
			        if ( $priority ) {
				        remove_action( 'woocommerce_after_add_to_cart_button', 'woocommerce_template_single_add_to_cart', $priority );
			        }
		        }
